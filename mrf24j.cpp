 ...
 */
 ...
 void Mrf24j::send16(word dest16, char * data, byte length) {
     ...
     // 0 | pan compression | ack | no security | no data pending | data frame[3 bits]
     write_long(i++, 0b00000001); // first byte of Frame Control with ACK
 ...
 void Mrf24j::send16ack(word dest16, char * data, byte length) {
     byte len = length; // get the length of the char* array 
     int i = 0;
     write_long(i++, bytes_MHR); // header length
     // +ignoreBytes is because some module seems to ignore 2 bytes after the header?!.
     ...
     // 0 | pan compression | ack | no security | no data pending | data frame[3 bits]
     write_long(i++, 0b00100001); // first byte of Frame Control with ACK    
